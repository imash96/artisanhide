...
import { FilterIcon, FunnelIcon, SlidersHorizontal, X } from "lucide-react";
  ...
  const filterChips = useMemo(() => {
    ...
    Object.entries(filters).forEach(([paramKey, valueSet]) => {
      ...
      Array.from(valueSet).forEach((optValue) => {
        ...
        const optLabel =
          category?.options.find((o) => o.value === optValue)?.label ??
          optValue;
        chipNodes.push(
          <FilterChip
    ...
    <section className="">
      <div className="flex items-center justify-between">
        <div className="text-sm font-light">
          Showing all {totalProduct} results
        </div>
      ...
      <FilterDrawer state={drawerOpen} toggle={toggleDrawer}>
        ...
        <div className="flex-1 px-4 my-4">
          <Accordion
            type="single"
            collapsible
            className="w-full space-y-2 border-t-0"
          >
            {filterList.map((item) => {
              ...
              return (
                <AccordionItem
                  ...
                  className={`border rounded-lg ${
                    hasActive ? "border-accent/80 bg-accent/5" : "border-border"
                  }`}
                >
                  ...
                  <AccordionContent className="px-4 pb-4">
                    <fieldset className="space-y-1 max-h-64 overflow-y-auto">
                      {item.options.map((opt) => {
                        ...
                        const checked = isSort
                          ? sort === opt.value
                          : categoryValues.has(opt.value);
...
function FilterOption({
  ...
  return (
    <label
      htmlFor={id}
      className="group flex items-center justify-between gap-2 text-sm cursor-pointer select-none p-2 rounded-md hover:bg-surface-elevated"
    >
      <div className="flex items-center gap-2">
        <div
          aria-hidden
          className={`w-4 h-4 rounded border flex items-center justify-center text-[10px] font-bold ${
            checked
              ? "bg-accent border-accent shadow-sm text-on-accent"
              : "border-muted group-hover:border-accent"
          }`}
        >
          {checked && (
            <span className="animate-in zoom-in-50 duration-150">✔</span>
          )}
        </div>
