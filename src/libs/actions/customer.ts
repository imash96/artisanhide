...
export const retrieveCustomer = async (): Promise<StoreCustomer | null> => {
    ...
    const headers = { ...authHeaders } as ClientHeaders
...
export const updateCustomer = async (body: StoreUpdateCustomer) => {
    const headers = { ...(await getAuthHeaders()) as ClientHeaders };

    const updateRes = await sdk.store.customer.update(body, {}, headers).then(({ customer }) => customer).catch(medusaError)
