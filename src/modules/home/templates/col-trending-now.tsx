...
export default async function TrendingNow({ region_id, }: { region_id: string }) {
    const products = await fetchProductByCollection({ collection_id: SALE_ID, limit: 12, region_id, fields: "*variants.calculated_price" }, "trending_now_collection")
    return (
        <SectionHeader title="Trending Now" desc="Discover Your Style: Navigate by Category for Effortless Shopping!" sectionName="on-sale">
